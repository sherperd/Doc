...
第 四 篇 Linux项目实践
第 13 章 项目实践：BT下载软件的开发
1、BitTorrent协议，简称BT。
2、Tim Berners Lee在1990年设计和发明了HTTP协议。HTTP之前，SMTP和FTP统治互联网。
3、2003年，年轻的软件工程师Bram Cohen发明了BitTorrent协议。
4、BT,是一个文件分发协议，每个下载者在下载的同时不断向其他下载者上传已下载的数据。特点：下载的人越多下载的速度越快，其原因在于每个下载者将已下载的数据提供给其他下载者下载，它充分利用了用户的上载带宽。BT协议通过一定的策略保证上传的速度越快，下载的速度也越快。
5、基于BT协议的文件分发系统构成：一个Web服务器；一个种子文件；一个Tracker服务器；一个原始文件提供者；一个网络浏览器；一个或多个下载者。
6、种子文件又称为元原文件或metafile。它保存了共享文件的一些信息，如共享文件的文件名、文件大小、Tracker服务器地址。通常很小，以.torrent为后缀。Tracker服务器保存着当前下载某共享文件的所有下载者的IP和端口。
7、种子文件就是提供者使用BT客户端生成的。把某个下载者本身称为客户端，把其他客户端称为peer。
8、BT客户端下载一个共享文件的过程：客户端首先解析种子文件，获取待下载的共享文件的一些信息，包括Tracker服务器的地址。然后客户端连接Tracker获取当前下载该文件的所有下载者的IP和端口。之后客户端根据IP和端口连接其他下载者，从它们那里下载文件，同时把自己已下载的部分提供给其他下载者下载。
9、共享文件在逻辑上被划分为大小不同的块，称为piece。每个块的大小通常为256KB。种子文件包含每个piece的hash值。BT协议规定使用Sha1算法对每个piece生成20字节的hash值，作为每个piece的指纹。
10、在实际上传和下载中，每个piece又被划分为大小不同的slice，每个slice的大小固定为16KB(16384字节)。peer之间每次传输slice为单位。
11、BT软件功能：解析种子文件获取待下载的文件的一些信息；连接Tracker获取peer的IP和端口，连接peer进行数据上传和下载、对要发布的提供共享文件制作和生成种子文件。
12、种子文件和Tracker返回的信息都以一种简单而高效的编码方式进行编码，称为B编码。客户端与Tracker交换信息基于HTTP协议，Tracker本身作为一个Web服务器存在。客户端与其他peer采用面向连接的可靠传输协议TCP进行通信。
13、B编码有4种类型：字符串、整数、列表、字典。4：spam，i3e，l4:spam4:eggse,d4:spaml3:aaa3:bbbee.
14、种子文件的结构，元信息文件或metafile，它是经过B编码的。种子文件事实上就是一个B编码的字典。
15、DHT，即分布式哈希表(Distribute Hash Table)，它是一种以分布式的方式来获取peer的方法。
16、BT种子文件网站：http://www.btchina.net，bt.greedland.net。Windows记事本分析。
17、与Tracker交互目的：将自己的下载进度告知Tracker以便Tracker进行一些相关的统计；获取当前下载同一个共享文件的peer的IP地址和端口号。
18、peer之间的协议，称为peer wire protocol，即peer连线协议。基于TCP的应用层协议。
19、客户端只给那些向它提供最快下载速度的4个peer上传数据。
20、握手信息是一个长度固定为68字节的消息。
21、一般从peer_id可识别出BT软件类型。
22、keep_alive消息用于维持连接，通常如果2分钟内没有向peer发送任何消息，则发送一个keep_alive信息。
23、客户端每隔一定时间，通常为10秒，计算一次各个peer的下载速度，如果某peer被解除阻塞，则发送unchoke消息。如果某个peer原先是解除阻塞的，而此次被阻塞，则发送choke消息。
24、关键算法和策略，流水线作业(提高数据传输的效率)，片断选择算法(提高下载速度至关重要)，阻塞算法，
25、事实上，HTTP协议的1.1版本就广泛地使用了流水线作业的思想，大大地提高了浏览器和Web服务器之间的传输效率。
26、片断选择算法，策略一(也被称为严格的优先级)，一旦向某个peer发送对某个piece中的slice的请求后，则该piece中的其他slice也从该peer处下载，这样可尽可能快地下载到一个完整地piece。策略二，最少优先。策略三，随机选择一个要下载的piece。策略四，最后阶段模式。
27、阻塞是一种临时拒绝上传的策略，虽然上传停止了，但下载仍然继续。
28、实践表明，10秒足以使下载速度达到最大。每个peer都拥有一个称为“optimistic unchoking(优化非阻塞)”peer。30秒足以让该peer的上载能力达到最大。
29、为了整体性能，每个peer在完成下载后应该作为种子存在一段时间，作为对整个系统的回报。
30、BT协议规范和“BT性能卓越的原因”。
31、系统结构设计，
32、频繁读写硬盘会使硬盘寿命缩短。
33、位图管理，BT协议采用位图指明当前哪些piece已经下载，哪些piece还没有下载。每个piece占一位。
